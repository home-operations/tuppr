...
| controller.logLevel | string | `"info"` | Controller log level. `debug` also switches to console-formatted output. tuppr never logs at warn, so `warn` and `error` both emit only errors. |
