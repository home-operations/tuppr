...
helm-lint: ## Run helm lint on charts
	helm lint charts/tuppr

.PHONY: helm-test
helm-test: ## Run helm unit tests
	helm unittest charts/tuppr
...
## Tool binaries are managed by mise (see .mise.toml). Override any of the
## variables below to point at a different location if you don't use mise.
KUBECTL ?= kubectl
KIND ?= kind
KUSTOMIZE ?= kustomize
...
# ENVTEST_K8S_VERSION is the version of Kubernetes to use for envtest binaries (e.g. 1.31).
...
.PHONY: setup-envtest
setup-envtest: $(LOCALBIN) ## Download envtest assets into the local bin directory.
