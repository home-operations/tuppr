...
func main() {
	...
	if secureMetrics {
		...
		certWatcher, err := certwatcher.New(
			filepath.Join(os.TempDir(), "k8s-metrics-server", "tls.crt"),
			filepath.Join(os.TempDir(), "k8s-metrics-server", "tls.key"),
		)
		if err != nil {
			setupLog.Error(err, "unable to create cert watcher for metrics server")
			os.Exit(1)
		}
		go func() {
			if err := certWatcher.Start(ctrl.SetupSignalHandler()); err != nil {
				setupLog.Error(err, "problem running cert watcher for metrics server")
			}
		}()
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "c2a7dd97.home-operations.com",
	...
	if err = (&controller.TalosPlanReconciler{
		...
		setupLog.Error(err, "unable to create controller", "controller", "Talos")
		os.Exit(1)
	}
	...
	if err = (&talupwebhook.TalosPlanValidator{
		...
		setupLog.Error(err, "unable to create webhook", "webhook", "Talos")
		os.Exit(1)
	}
