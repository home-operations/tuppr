...
// KubectlSpec defines configuration for the kubectl container used for draining
type KubectlSpec struct {
	// Image is the container image to use for kubectl commands
	// +kubebuilder:default="bitnami/kubectl:latest"
	// +optional
...
type TalosUpgradeSpec struct {
	...
	// Kubectl configuration for drain operations
	// +optional
